	...
	* */
	...
	self.flushCache = function() {
  	...
  	self.htmlDocument = function () {
		...
		if (htmlCache == "") {
			if (self.config.htmlDocument){
				htmlCache = self.config.htmlDocument;
			} else {
				htmlCache = self.grab(self.config.url, self.config.encoding, self.config.userAgent);
			}
		}
		return htmlCache;
	};
...
Job.prototype.get = function(schema) {
	...
	if(schema) {
		...
		// allow to pass on htmlDocument
		if(schema.htmlDocument) {
			self.config.htmlDocument = schema.htmlDocument;
		}
