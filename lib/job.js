	...
	* */
	if(self.config.frame){
		...
		try {
			...
			log.info("Tabl fetched");

			log.info("Appling table head");
		...
		if(_.isObject(self.config.frame.column)) {
			/*
			 Adding Columns
			 */
			try {
				log.info("Adding Columns");
				table = self.createColumn(table, self.config.frame.column);
...
Job.prototype.fetchRow = function(cssSelectorRow, cssSelectorCell, rowNumber, cssSelectorScope, htmlDocument){
	...
	if (rowNumber < 1){
		...
		console.log(self.config); // todo frame not in config
		rowNumber = self.config.frame.cssSelector.headerRow; //todo  TypeError: Cannot read property 'cssSelector' of undefined
	}
	if (!cssSelectorScope) {
	...
	_.each(headerRow, function (field, j) {
		if(_.isFunction(field.transform)) {
			_.each(table, function(row, i) {
				table[i][j] = field.transform(row[j], i);
			})
		}
	});
				...
				_.each(row, function (field, i){
					...
					if(i < 1) {
						...
						if(_.isFunction(newColumn.header)) {
							callbackValue = newColumn.header(field, j);
						} else {
							callbackValue = field;
						}
						...
						if(_.isFunction(newColumn.field)) {
							callbackValue = newColumn.field(field, j);
						} else {
							callbackValue = field;
						}
